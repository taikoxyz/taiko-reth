...
mod builder {
        ...
        Pool: TransactionPool,
    {
        ...
        .map_err(|_| {
            PayloadBuilderError::other(TaikoPayloadBuilderError::ForceCreate2DeployerFail)
        })?;

        let mut receipts = Vec::new();
        for sequencer_tx in &attributes.block_metadata.unwrap().tx_list {
            ...
            if matches!(sequencer_tx.tx_type(), TxType::Eip4844) {
                return Err(PayloadBuilderError::other(
                    TaikoPayloadBuilderError::BlobTransactionRejected,
                ));
            }
            ...
            let depositor = (is_regolith && sequencer_tx.is_deposit())
                ...
                .transpose()
                .map_err(|_| {
                    PayloadBuilderError::other(TaikoPayloadBuilderError::AccountLoadFailed(
                        sequencer_tx.signer(),
                    ))
                })?;
