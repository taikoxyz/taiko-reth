...
use reth_primitives::{constants::EMPTY_ROOT_HASH, keccak256, B256};
    ...
    pub fn hash(&self) -> B256 {
        match self.data {
                ...
                .get_or_insert_with(|| self.calc_reference())
            {
                ...
                MptNodeReference::Bytes(bytes) => keccak256(bytes).into(),
            },
        }
    }
        ...
        match &self.data {
            ...
            _ => {
                ...
                if encoded.len() < 32 {
                    ...
                    MptNodeReference::Digest(keccak256(encoded).into())
                }
            }
        }
    }
