...
impl Database for MemDb {
    ...
    fn block_hash(&mut self, number: U256) -> Result<B256, Self::Error> {
        let block_no: u64 = number.try_into().map_err(|_| {
            eyre::eyre!("invalid block number: expected <= {}, got {}", u64::MAX, &number)
        })?;
        self.block_hashes.get(&block_no).cloned().ok_or(DbError::BlockNotFound(block_no))
    }
}
